function [ gaussian2DParameters ] = IIT_G2DFit_gaussian2DFittingRotational( data, iterationsNumber, pixelSize)
%IIT_G2DFit_gaussian2DFittingSupervised Fits 2D data to a gaussian function.
...
coeff = zeros(6,1); oldCoeff = coeff;
...
for currentIteration = 1 : iterationsNumber
    ...
    if (isnan(condNum) || condNum < minimumCondNumber)
        gaussian2DParameters.sx = -1;
        gaussian2DParameters.sy = -1;
        gaussian2DParameters.ux = -1;
        gaussian2DParameters.uy = -1;
        gaussian2DParameters.fwhmX = -1;
        gaussian2DParameters.fwhmY = -1;
        gaussian2DParameters.peak = -1;
        gaussian2DParameters.kValue = -1;
        gaussian2DParameters.rSquare = -1;
        gaussian2DParameters.background = -1;
        gaussian2DParameters.iterations = -1;
        gaussian2DParameters.theta = -1;
        return;
    end
...
gaussian2DParameters.sx = sx;
gaussian2DParameters.sy = sy;
gaussian2DParameters.ux = ux;
gaussian2DParameters.uy = uy;
gaussian2DParameters.fwhmX = fwhmX;
gaussian2DParameters.fwhmY = fwhmY;
gaussian2DParameters.peak = peak;
gaussian2DParameters.kValue = kValue;
gaussian2DParameters.rSquare = rSquare;
gaussian2DParameters.background = backgroudLevel;
gaussian2DParameters.iterations = currentIteration;
gaussian2DParameters.theta = theta;

% if (show)
%     fitting=zeros(height, width);
%     for y = 1:size(fitting,1)
%         for x = 1:size(fitting,2)
%             fitting(y,x) = exp(sum(coeff'.*[1 x y x^2 y^2 x*y]));
%         end
%     end
% 
%     imtool(fitting, [min(min(fitting)), max(max(fitting))]);
% end
