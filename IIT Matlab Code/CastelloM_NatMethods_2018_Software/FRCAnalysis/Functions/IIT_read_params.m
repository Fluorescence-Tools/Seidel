function [parout, given] = IIT_read_params(params, args)
...
%
%   Comments:
%       If a function uses IIT_read_params, but the complete parameter
...
for i = 1:2:size(args,2)
    if strcmp(args{i}, 'params')
        if not(strcmp(class(args{i+1}), 'struct'))
    ...
    elseif strcmp(args{i}, 'varargin')
        parout = IIT_read_params(params, args{i + 1});
    elseif isfield(params, args{i})
        parout.(args{i}) = args{i + 1};
        given = [given, args{i}];
    else
        if not(strcmp(class(args{i}), 'char'))
