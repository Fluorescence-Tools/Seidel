/************************************************************************
  ...
  PicoQuant Unified TTTR (PTU)    File Access Demo in C
...
// check SPT64 Manual for Tag dictionary (Appendices/Tag Dictionary)
...
#define FileTagEnd         "Header_End"               // has always to be appended as last tag (BLOCKEND)
...
// TDateTime (in file) to Time_t (standard C) conversation
...
unsigned long long Nphotons= 0;
long long truensync, truetime, MT;
int m, c, CHAN, TAC;
double GlobRes = 0.0;
...
{
	  MT= TimeTag; CHAN= Channel; TAC= DTime; Nphotons+=1;
/*  if(IsT2)
  {
    CHAN= Channel; MT= TimeTag; TAC= DTime;
  }
  else
  {
    CHAN= Channel; MT= TimeTag; TAC= DTime;
...
{
   MT=TimeTag; CHAN= Markers+64; TAC= 0; Nphotons+=1;
}
...
// Bestimmt Länge eines files (Länge eines jeden Elements=size)  //
////////////////////////////////Read PTU format////////////////////////////////////////////////////

__int64 PQ_ptu_sf(char *filename, long long length, long long *eventN, int *tac, long long *t, 
				  unsigned __int8 *can, unsigned long long &j, unsigned long long ov_in, int stage)
{
	  char Magic[8];
	  char Version[8];
	  char Buffer[40];
	  ...
	  long long NumRecords = -1;
		...
		oflcorrection = ov_in; Nphotons=0;

	  if((fpin=fopen(filename,"rb"))==NULL) goto ex;

	  Result = fread( &Magic, 1, sizeof(Magic) ,fpin);
	  if (Result!= sizeof(Magic)) goto close;
	  
	  Result = fread(&Version, 1, sizeof(Version) ,fpin);
	  if (Result!= sizeof(Version)) goto close;
	  
	  if (strncmp(Magic, "PQTTTR", 6)) goto close;

	  // read tagged header
	  do
	  { 
			Result = fread( &TagHead, 1, sizeof(TagHead) ,fpin);
		  if (Result!= sizeof(TagHead)) goto close;
	...
	// End Header loading
	  ...
	  for(RecNum=0;RecNum<NumRecords;RecNum++)
	  {
		...
		if ((Nphotons>0) && (Nphotons<=NumRecords))
		{
				tac[Nphotons-1] = TAC; t[Nphotons-1] = MT;
			eventN[Nphotons-1] = Nphotons-1; can[Nphotons-1] = CHAN; 
		}
	  }
	...
	close:
	  ...
	  j=Nphotons;
	ex:
	  return(oflcorrection);
	}	

long long PQ_ptuHeader_sf(char *filename, char *filename_out)
	{
	  char Magic[8];
	  char Version[8];
	  char Buffer[40];
	  ...
	  if((fpin=fopen(filename,"rb"))==NULL) goto ex;
	  if((fpout=fopen(filename_out,"w"))==NULL) goto ex;
  ...
  do
  {
    ...
    if (TagHead.Idx > -1)
    {
      sprintf_s(Buffer, sizeof(Buffer), "%s(%d)", TagHead.Ident,TagHead.Idx);
    }*/
    fprintf(fpout, "\n%-40s", Buffer);
    switch (TagHead.Typ)
    {
      ...
      case tyTDateTime:
        time_t CreateTime;
        CreateTime = TDateTime_TimeT(*((double*)&(TagHead.TagValue)));
        fprintf(fpout, "%s", ctime(&CreateTime), "\0");
        break;
      case tyAnsiString:
        AnsiBuffer = (char*)calloc((size_t)TagHead.TagValue,1);
                Result = fread(AnsiBuffer, 1, (size_t)TagHead.TagValue, fpin);
...
int main(int argc, char **argv)
{
	...
	char outname[100] = "K:\\vanderVoortN\\python\\FRC\\fout.txt";
	char inname[100] = "K:\\vanderVoortN\\python\\FRC\\PQSpcm_2019-02-01_16-37-57.ptu";
	char* ptoutname = outname;
	char* ptinname = inname;
	
	long long NumRecords = PQ_ptuHeader_sf(ptinname, ptoutname);
	printf("returned value is %d\n", NumRecords);
	printf("Hello, I'm about to initiliaze some values\n");
	long long length;
	long long * pteventN = new long long[NumRecords];
	long long * ptt = new long long [NumRecords];
	int * pttac = new int [NumRecords];
	unsigned __int8 * ptcan = new unsigned __int8 [NumRecords];
	unsigned long long j;
	unsigned long long ov_in;
	int stage;

	oflcorrection = PQ_ptu_sf(ptinname, length, pteventN, pttac, ptt, ptcan, j, ov_in, stage);
	printf("Hello, I'm about to runPQ PTU sf\n");
	printf("oflcorrection is: %lli", oflcorrection);
	fclose(fpout);
	
	int long long i = 0;
	for( i =0; i<100; i++)
	{
		if(ptcan[i]>66)	printf("value of can is %d for i equals %lli\n ", ptcan[i], i );
		printf("value of t is %d for i equals %lli\n", ptt[i], i);
	}
